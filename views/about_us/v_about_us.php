<div class="page-title-area">
    <div class="container">
        <div class="page-title-content">
            <h1>Giới Thiệu</h1>
            <ul>
                <li><a href="index.php">Trang chủ</a></li>
                <li>Giới thiệu</li>
            </ul>
        </div>
    </div>
</div>


<div class="about-area pt-100 pb-75">
    <div class="container">
        <div class="row align-items-center">
            <div class="col-lg-3 col-md-12">
                <div class="about-image">
                    <img src="public/patoi/assets/img/about/about1.jpg" alt="about-image">
                </div>
            </div>
            <div class="col-lg-6 col-md-12">
                <div class="about-text">
                    <h2>Câu Chuyện Về Hành Trình Của Chúng Tôi</h2>
                    <span>Hiếu và những người bạn</span>
                    <p>Chúng tôi cung cấp các sản phẩm chất lượng với giá thấp mỗi ngày. 
                        Chúng tôi đã làm việc trong ngành này trong hầu hết 20 năm. 
                        Chúng tôi đã mua sắm trực tuyến rất tâm sự . 
                        Khách hàng của chúng tôi thích và tin tưởng chúng tôi rất nhiều vì chất lượng sản phẩm của trang web của chúng tôi.</p>
                </div>
            </div>
            <div class="col-lg-3 col-md-12">
                <div class="about-image">
                    <img src="public/patoi/assets/img/about/about2.jpg" alt="about-image">
                </div>
            </div>
        </div>
    </div>
</div>


<div class="facility-area pb-75">
    <div class="container">
        <div class="row">
            <div class="col-lg-3 col-md-6 col-sm-6 col-6">
                <div class="facility-box">
                    <img src="public/patoi/assets/img/icon/icon1.png" alt="icon">
                    <h3>Sản phẩm chát lượng</h3>
                </div>
            </div>
            <div class="col-lg-3 col-md-6 col-sm-6 col-6">
                <div class="facility-box bg-fed3d1">
                    <img src="public/patoi/assets/img/icon/icon2.png" alt="icon">
                    <h3>Giao hàng nhanh</h3>
                </div>
            </div>
            <div class="col-lg-3 col-md-6 col-sm-6 col-6">
                <div class="facility-box bg-a9d4d9">
                    <img src="public/patoi/assets/img/icon/icon3.png" alt="icon">
                    <h3>Hỗ trợ 24/7</h3>
                </div>
            </div>
            <div class="col-lg-3 col-md-6 col-sm-6 col-6">
                <div class="facility-box bg-fef2d1">
                    <img src="public/patoi/assets/img/icon/icon4.png" alt="icon">
                    <h3>Thanh toán an toàn</h3>
                </div>
            </div>
        </div>
    </div>
</div>


<div class="feedback-area pb-100">
    <div class="container">
        <div class="section-title">
            <h2>What Our Client Says</h2>
        </div>
        <div class="row align-items-center">
            <div class="col-lg-6 col-md-12">
                <div class="feedback-slides owl-carousel owl-theme">
                    <div class="single-feedback-box">
                        <p>“Đã thử ăn và thấy rất ngon”</p>
                        <div class="client-info">
                            <img src="public/patoi/assets/img/user/anh.jpg" alt="user">
                            <h3>Phạm Thị Ánh</h3>
                            <span>Khách</span>
                        </div>
                    </div>
                    <div class="single-feedback-box">
                        <p>“Giao hàng quá nhanh. Mùng 4 đặt hàng mà sáng mùng 5 đã nhận được hàng nhưng mà là mùng 5 tháng sau.”</p>
                        <div class="client-info">
                            <img src="public/patoi/assets/img/user/mphuong.jpg" alt="user">
                            <h3>Nguyễn Minh Phương</h3>
                            <span>Khách</span>
                        </div>
                    </div>
                    <div class="single-feedback-box">
                        <p>“OK!”</p>
                        <div class="client-info">
                            <img src="public/patoi/assets/img/user/hieu.jpg" alt="user">
                            <h3>Nông Minh Hiếu</h3>
                            <span>Founder and CEO</span>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-lg-6 xol-md-12">
                <div class="video-box">
                    <img src="public/patoi/assets/img/video.jpg" alt="video-image">
                    <a href="https://www.youtube.com/watch?v=6t-MjBazs3o" class="popup-video"><i class='bx bx-play'></i></a>
                </div>
            </div>
        </div>
    </div>
</div>


<div class="team-area pb-75">
    <div class="container">
        <div class="section-title">
            <h2>Những Người Sáng Lập</h2>
        </div>
        <div class="row justify-content-center">
            <div class="col-lg-4 col-md-6 col-sm-6">
                <div class="single-team-member">
                    <img src="public/patoi/assets/img/team/hieu.jpg" alt="team-image">
                    <h3>Nông Minh Hiếu</h3>
                    <span>Founder, CEO, BA, Dev, Tester</span>
                </div>
            </div>
            <div class="col-lg-4 col-md-6 col-sm-6">
                <div class="single-team-member">
                    <img src="public/patoi/assets/img/team/anh.jpg" alt="team-image">
                    <h3>Phạm Thị Ánh</h3>
                    <span>Chắp tay chỉ đạo</span>
                </div>
            </div>
            <div class="col-lg-4 col-md-6 col-sm-6">
                <div class="single-team-member">
                    <img src="public/patoi/assets/img/team/mphuong.jpg" alt="team-image">
                    <h3>Nguyễn Minh Phương</h3>
                    <span>Chỉ đạo cùng Ánh</span>
                </div>
            </div>
        </div>
    </div>
</div>


<div class="partners-area pb-100">
    <div class="container">
        <div class="section-title">
            <h2>Đối Tác Của Chúng Tôi</h2>
        </div>
        <div class="partners-inner">
            <div class="row justify-content-center align-items-center">
                <div class="col-lg-2 col-md-3 col-sm-4 col-4">
                    <div class="single-partners-box">
                        <a href="shop_girl.php" class="d-block">
                            <img src="public/patoi/assets/img/brands/brands1.png" alt="brands">
                        </a>
                    </div>
                </div>
                <div class="col-lg-2 col-md-3 col-sm-4 col-4">
                    <div class="single-partners-box">
                        <a href="shop_girl.php" class="d-block">
                            <img src="public/patoi/assets/img/brands/brands2.png" alt="brands">
                        </a>
                    </div>
                </div>
                <div class="col-lg-2 col-md-3 col-sm-4 col-4">
                    <div class="single-partners-box">
                        <a href="shop_girl.php" class="d-block">
                            <img src="public/patoi/assets/img/brands/brands3.png" alt="brands">
                        </a>
                    </div>
                </div>
                <div class="col-lg-2 col-md-3 col-sm-4 col-4">
                    <div class="single-partners-box">
                        <a href="shop_girl.php" class="d-block">
                            <img src="public/patoi/assets/img/brands/brands4.png" alt="brands">
                        </a>
                    </div>
                </div>
                <div class="col-lg-2 col-md-3 col-sm-4 col-4">
                    <div class="single-partners-box">
                        <a href="shop_girl.php" class="d-block">
                            <img src="public/patoi/assets/img/brands/brands5.png" alt="brands">
                        </a>
                    </div>
                </div>
                <div class="col-lg-2 col-md-3 col-sm-4 col-4">
                    <div class="single-partners-box">
                        <a href="shop_girl.php" class="d-block">
                            <img src="public/patoi/assets/img/brands/brands6.png" alt="brands">
                        </a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
